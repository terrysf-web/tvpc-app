...
// 예배 순서가 끝났음을 알리는 줄 — 일어서기 안내, 그리고 그 아래 이어지는
// 새벽예배·금요영성집회 본문표(요일 칸 포함).
const ORDER_END =
  /^[*✻＊]?\s*(표는|일어서|새벽\s*예배|금요\s*영성집회|생명의\s*삶)|^[월화수목금토일]\s*\(\s*\d{1,2}\s*일\s*\)/;
...
function extractOrderAndSermon(lines) {
  ...
  for (let i = 0; i < cleaned.length; i++) {
    ...
    // 바랍니다" 같은 안내와 새벽예배·금요영성집회 본문표가 이어지는데,
    // 라벨이 없는 줄이라 그냥 두면 마지막 항목(축도) 내용으로 끌려 들어간다
    // — 2026-09-13 주보에서 축도 칸에 한 주치 새벽예배 본문표가 통째로
    // 들어가 엉망이 됐다. 안내가 시작되면 거기서 멈춘다.
    //
    // 예전에는 '✻'가 붙은 줄만 안내로 봤는데, 별표를 안 쓰거나 다른 글자를
    // 쓴 주보에서는 그냥 지나쳐 버렸다. 이제 별표 여부와 상관없이 본다.

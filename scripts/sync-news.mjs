/**
 * 교회 홈페이지(tvpc.church) → Firestore `events` 자동 동기화 (+ `news`의
 * "행사" 카드).
 *
 ...
 *
 * "교회 소식"(공지)은 더 이상 여기서 홈페이지 게시판을 긁어오지 않는다 —
 * 관리자가 홈페이지에 따로 올리는 걸 깜빡하면 그대로 방치되는 문제가 있어,
 * 매주 반드시 새로 나오는 주보 쪽(scripts/sync-bulletin.mjs의
 * syncNoticesToNews)에서만 채운다. 이 스크립트는 예전 방식으로 긁어온
 * 옛 공지 문서만 정리한다.
 *
 * 로컬 실행:
...
console.log(`완료: 일정 ${eventsWrote}건 동기화`);
if (eventsWrote === 0) {
  console.log('가져온 일정이 없습니다 — 위 탐색 로그에서 소스 상태를 확인하세요.');
}
process.exit(0);

/**
 * 일회성 — 실수로 지웠던 홈페이지발 "교회 소식" 카드를 되살린다.
 * sync-news.mjs에서 뺀 옛 스크레이핑 로직(commit 33a878e 버전) 그대로를
 * 한 번만 돌려서, 홈페이지에 지금도 남아 있는 공지를 다시 긁어와
 * web-{hash} id로 채운다 — 실제 홈페이지 내용을 그대로 가져오는 것이라
 * 지워지기 전 문서와 똑같은 id로 복원된다. 이후 sync-news.mjs는 계속
 * 이 소식을 갱신하지 않으므로(요청대로) 이 스크립트도 이번 한 번만 쓰고
 * 지운다.
 *
 * 로컬 실행:
 *   FIREBASE_SERVICE_ACCOUNT="$(cat serviceAccount.json)" node scripts/restore-web-news.mjs
 */
